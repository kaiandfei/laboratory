<template>
  <div>
    ...
    <detail ref="detailArea" :detailShow="pageState === 2" :subjectName="subjectName" @back="back()"></detail>
    <Account
      ref="accountArea"
...
const initData = () => {
  getAccountList({ type: 2, subject_id: subjectID.value }).then((res: any) => {
...
// 开始建帐或继续建帐
const accountCreate = (ID: any, value: string, name: string, subject_id: any) => {
  subjectName.value = name
  title.value = value
  pageState.value = 1
  selectId.value = ID
...
const finishAccount = (ID: number) => {
  endAccount({ id: ID, type: 2 }).then((res) => {
    if (res.code === 10) {
      initData()
    }
  })
...
const exportExcel = () => {
  ...
  axios
    ...
    .then((res) => {
      ...
      const fileName = store.state.login.userInfo.school.name + '-' + '期初设备建帐.xlsx'
