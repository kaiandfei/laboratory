    ...
    <div class="leftArea">
      ...
      <div class="topArea" v-show="conserveShow === true">
        ...
        <div class="nav" style="height: 80px">
          <div class="name">损坏原因:</div>
          <div>
            <el-input type="textarea" :row="2" v-model="compensateRegistrationInfo.damage_desc"></el-input>
          </div>
        </div>
        <div class="nav">
          <div class="name">赔偿人:</div>
          <div>
            <el-select v-model="compensateRegistrationInfo.repay_id">
              <el-option
                v-for="item in optionUser"
                :key="item.id"
                :label="item.username"
        ...
        <div class="nav">
          ...
          <div>
            <el-input v-model="compensateRegistrationInfo.repay_price"></el-input>
          </div>
        </div>
        <div class="nav">
          ...
          <div>
            <el-date-picker
              ...
              type="date"
              v-model="compensateRegistrationInfo.repay_time"
            ></el-date-picker>
          </div>
        </div>
        <div class="nav" style="height: 80px">
          ...
          <div>
            <el-input v-model="compensateRegistrationInfo.repay_desc" type="textarea" :rows="2"></el-input>
          </div>
        </div>
      </div>
      <div class="topArea" v-show="conserveShow === false">
        <div class="nav">
          ...
          <div>
            {{ compensateRegistrationInfo.damage_man }}
          </div>
        </div>
        <div class="nav" style="float: right; height: 200px; display: block; width: 40%">
          <div class="name" style="float: left; height: 10%">图片：</div>
          <div v-for="(item, index) in compensateRegistrationInfo.url" :key="index">
            <img :src="item" />
          </div>
        </div>
        <div class="nav">
          ...
          <div>
            {{ compensateRegistrationInfo.damage_desc }}
          </div>
        </div>
        <div class="nav">
          <div class="name">赔偿人:</div>
          <div>
            {{ compensateRegistrationInfo.repay_man }}
          </div>
        </div>
        <div class="nav">
          <div class="name">赔偿金额:</div>
          <div>
            {{ compensateRegistrationInfo.repay_price }}
          </div>
        </div>
        <div class="nav">
          <div class="name">赔偿时间:</div>
          <div>
            {{ compensateRegistrationInfo.repay_time }}
          </div>
        </div>
        <div class="nav">
          <div class="name">赔偿说明:</div>
          <div>
            {{ compensateRegistrationInfo.repay_desc }}
          </div>
        </div>
      </div>
      <div class="title2">仪器位置及数量清单</div>
      <div class="bottomArea" v-show="conserveShow === true">
        <div class="tableArea">
          <el-table
            :data="compensateRegistrationInfo.list"
            style="width: 100%"
            height="300"
          ...
          >
            ...
            <el-table-column label="名称" :show-overflow-tooltip="true" prop="name"></el-table-column>
            ...
            <el-table-column label="操作" prop="">
              <template #default="scope">
                <div @click="remove(scope.$index)" style="cursor: pointer; color: green">删除</div>
              </template>
            </el-table-column>
          </el-table>
        </div>
      ...
      <div class="containerArea">
        <div class="infoArea" v-for="item in instrumentList" :key="item.id">
          ...
          <div class="tableArea" v-show="item.stockStatus === true">
            ...
            >
              <el-table-column label="序号" type="index"></el-table-column>
              <el-table-column label="学校编号" prop="hardware_number" show-overflow-tooltip></el-table-column>
...
import { instrumnetData, addDamageRegistrationData } from '@/service/instrument-manage/damageRegistration/types'
import {
  getInstrumentInfo,
  postAddDamageInfo,
  getTeacherList
...
const dataMap = reactive({
  compensateRegistrationInfo: addDamageRegistrationData,
...
let { instrumentList, week } = toRefs(dataMap)

school_id.value = JSON.parse(sessionStorage.getItem('state') || '').login.userInfo.school_id

const compensateRegistrationInfo: any = ref({})

const initData = () => {
  compensateRegistrationInfo.value = {}
  compensateRegistrationInfo.value.type = 1
  compensateRegistrationInfo.value.list = []
  compensateRegistrationInfo.value.url = []
  compensateRegistrationInfo.value.repay_man = ''
  compensateRegistrationInfo.value.repay_price = ''
  compensateRegistrationInfo.value.damage_desc = ''
  compensateRegistrationInfo.value.repay_desc = ''
  compensateRegistrationInfo.value.status = 1
...
const uploadSuccess = (response: unknown, file: UploadFile, fileList: UploadFile[]) => {
  ...
  compensateRegistrationInfo.value.url = urlData
}
...
const addDamage = (info: any) => {
  if (compensateRegistrationInfo.value.list.some((e: any) => e.id === info.id)) {
    ElMessageBox.alert('当前仪器已在清单中!', '提示', {
      confirmButtonText: '确认'
    })
    return
  }
  compensateRegistrationInfo.value.list.push({
    id: info.id,
...
const submit = () => {
  if (!compensateRegistrationInfo.value.damage_id) {
    ElMessage({
      type: 'warning',
      message: '请选择损坏人'
    })
  ...
  if (compensateRegistrationInfo.value.list.length === 0) {
    ElMessage({
      type: 'warning',
      message: '请选择仪器'
    })
  ...
  postAddDamageInfo(compensateRegistrationInfo.value).then((res) => {
    if (res.code === 10) {
...
const remove = (pot: number) => {
  compensateRegistrationInfo.value.list.splice(pot, 1)
}

// 获取损坏用户id对应的用户名
const damagerSel = (data: any) => {
  compensateRegistrationInfo.value.damage_man = data.username
}

// 获取赔偿人用户id对应的用户名
const payerSel = (data: any) => {
  compensateRegistrationInfo.value.repay_man = data.username
}
