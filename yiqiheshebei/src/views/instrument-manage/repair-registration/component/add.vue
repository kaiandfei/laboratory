<template>
  <div v-show="addState">
    <div class="leftArea">
      <div class="head-area">
        <div class="title">维修、保养登记</div>
        <div class="buttonArea">
          <el-button v-show="conserveShow === false" type="primary" @click="conserveShow = true">新增维修卡</el-button>
          <el-button v-show="conserveShow === true" type="info">新增维修卡</el-button>
          <el-button type="primary" @click="back()">返回</el-button>
          <el-button type="primary" v-show="conserveShow === true" @click="commit()">保存</el-button>
          <el-button type="info" v-show="conserveShow === false">保存</el-button>
        </div>
      </div>
      <div class="title2">维修、保养登记卡</div>
      <div class="topArea" v-show="conserveShow === true">
        <div class="first">
          <div class="nav nav-type">
            <div class="name is-must-div">类型:</div>
            <div>
              <el-select v-model="repairRegistrationInfo.mode" @change="changeType(repairRegistrationInfo.mode)">
                <el-option
        ...
        <div class="nav">
          ...
          <div>
            <el-input v-model="repairRegistrationInfo.factory"></el-input>
          </div>
        </div>
        <div class="nav">
          ...
          <div>
            <el-date-picker
              ...
              v-model="repairRegistrationInfo.start_time"
            ></el-date-picker>
          </div>
        </div>
        <div class="nav">
          ...
          <div>
            <el-date-picker
              ...
              value-format="YYYY-MM-DD"
              type="date"
              v-model="repairRegistrationInfo.plan_end_time"
            ></el-date-picker>
          </div>
        </div>
        <div class="nav" style="height: 80px">
          ...
          <div>
            <el-input v-model="repairRegistrationInfo.desc" type="textarea" :rows="2"></el-input>
          </div>
        </div>
      </div>
      <div class="topArea" v-show="conserveShow === false">
        ...
        <div class="nav" style="float: right; height: 200px; display: block; width: 40%">
          ...
          <div v-for="(item, index) in repairRegistrationInfo.url" :key="index">
            <img :src="item" />
          </div>
        </div>
        <div class="nav">
          ...
          <div>
            {{ repairRegistrationInfo.factory }}
          </div>
        </div>
        <div class="nav">
          <div class="name">开始时间:</div>
          <div>
            {{ repairRegistrationInfo.start_time }}
          </div>
        </div>
        <div class="nav">
          <div class="name">预计完成时间:</div>
          <div>
            {{ repairRegistrationInfo.plan_end_time }}
          </div>
        </div>
        <div class="nav">
          <div class="name">损坏情况:</div>
          <div>
            {{ repairRegistrationInfo.desc }}
          </div>
        </div>
      </div>
      <div class="titleArea">仪器位置及数量清单</div>
      <div class="bottomArea" v-show="conserveShow === true">
        <div class="tableArea">
          <el-table
            :data="repairRegistrationInfo.list"
            style="width: 100%"
            height="280"
...
import { instrumnetData, addRepairInfo, addRepairData } from '@/service/instrument-manage/repairRegistration/types'
import { getInstrumentInfo, postAddRepairInfo } from '@/service/instrument-manage/repairRegistration/index'
...
const dataMap = reactive({
  repairRegistrationInfo: addRepairData,
...
const repairRegistrationInfo: any = ref({})

const addRepair = (info: any) => {
  if (repairRegistrationInfo.value.list.some((e: any) => e.id === info.id)) {
    ElMessageBox.alert('当前仪器已在清单中!', '提示', {
      confirmButtonText: '确认'
    })
    return
  }
  repairRegistrationInfo.value.list.push({
    id: info.id,
...
const initData = () => {
  ...
  repairRegistrationInfo.value.type = 1
  repairRegistrationInfo.value.list = []
  repairRegistrationInfo.value.desc = ''
  repairRegistrationInfo.value.url = []
}
...
const changeType = (index: any) => {
  optionType.value.forEach((e: any) => {
    if (e.value === index) {
      typeName.value = e.label
    }
  })
}
...
const uploadSuccess = (response: unknown, file: UploadFile, fileList: UploadFile[]) => {
  ...
  repairRegistrationInfo.value.url = urlData
}
...
const commit = () => {
  if (!repairRegistrationInfo.value.mode) {
    ElMessage({
      type: 'warning',
      message: '请选择类型'
    })
  ...
  if (!repairRegistrationInfo.value.factory) {
    ElMessage({
      type: 'warning',
      message: '请输入负责企业'
    })
  ...
  if (!repairRegistrationInfo.value.start_time) {
    ElMessage({
      type: 'warning',
      message: '请选择开始时间'
    })
  ...
  if (!repairRegistrationInfo.value.plan_end_time) {
    ElMessage({
      type: 'warning',
      message: '请选择预计完成时间'
    })
  ...
  if (repairRegistrationInfo.value.list.length === 0) {
    ElMessage({
      type: 'warning',
      message: '请选择仪器'
    })
  ...
  postAddRepairInfo(repairRegistrationInfo.value).then((res) => {
    if (res.code === 10) {
...
const remove = (pot: number) => {
  repairRegistrationInfo.value.list.splice(pot, 1)
}
