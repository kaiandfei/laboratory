  ...
  <main v-show="accountShow">
    ...
    <div class="searchArea">
      <div class="area">
        ...
        <div class="button2" @click="addIns">新建仪器</div>
      </div>
      <div class="search">
    ...
    <div class="tableArea">
      ...
      >
        <el-table-column align="left" type="index" label="序号" fixed="left"></el-table-column>
        <el-table-column
          align="left"
          prop="number"
          label="分类编号"
        ...
        >
          <template #default="scope">
            <input class="inputArea" v-model="scope.row.asset_number" @blur="changeInput(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column
          align="left"
        ...
        <!-- <el-table-column align="left" prop="" label="应配数量" width="100">
          <template #default="scope">
        ...
        <el-table-column label-class-name="is-must" align="left" prop="price" label="金额" width="100">
          <template #default="scope">
            <input class="inputArea" v-model.number="scope.row.price" type="number" @blur="changeInput(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column label-class-name="is-must" align="left" prop="" label="期初数量" width="100">
          <template #default="scope">
            <input class="inputArea" v-model.number="scope.row.num" type="number" @blur="changeInput(scope.row)" />
          </template>
        </el-table-column>
        <el-table-column label-class-name="is-must" align="left" prop="" label="位置" width="400">
          <template #default="scope">
            <select
  ...
  <el-drawer v-model="drawer" title="添加仪器">
    <div class="search-box">
      <div class="searchInput">
        ...
        <img src="@assets/images/common/search.png" alt="搜索" @click="getSearchData" />
      </div>
    </div>
    <el-table
...
const initData = (ID: number, subject_id: number) => {
  tableData.value = []
  ...
  getAccountInfo({
    ...
    .then((res: any) => {
      ...
      console.log(pageData.value.total)
      infoTable()
    })
    .finally(() => {
...
const getTableList = () => {
  ...
  getAccountInfo({
    ...
    .then((res: any) => {
      ...
      //   if (res.data[key].detail.length === 0) {
      //     tableData.value.push(
      //       Object.assign(res.data[key], { can_del: false, cost_type: res.data[key].cost_type, url: res.data[key].url })
      //     )
      //   } else {
      //     for (const n in res.data[key].detail) {
      //       tableData.value.push(
      //         Object.assign(res.data[key].detail[n], {
      //           type: 1,
      //           can_del: true,
      //           cost_type: res.data[key].cost_type,
...
const endAccountCreation = () => {
  ElMessageBox.confirm('如未填写入库位置，则该数据不会入库！是否确认继续', '确认', {
    ...
    .then(() => {
      endAccount({ id: selectId.value, type: 1 }).then((res) => {
        if (res.code === 10) {
          emit('back')
        }
      })
  ...
  // let pulData: any = []
  // tableData.value.forEach((e: any, index: any) => {
  //   if (e.laboratory_id !== '') {
  //     pulData.push(tableData.value[index])
  //   }
  // })
  // saveAccountInfo({ accounts_id: selectId.value, detail: pulData, type: 2 }).then((res) => {
  //   if (res.code === 10) {
  //     endAccount({ id: selectId.value, type: 1 }).then((res) => {
  //       if (res.code === 10) {
  //         emit('back')
...
const back = () => {
  ...
  //     return
  //   }
  //   saveAccountInfo({ accounts_id: selectId.value, detail: pulData, type: 1 }).then((res) => {
  //     if (res.code === 10) {
  //       emit('back')
  //     }
  //   })
