<template>
  ...
  <main>
      ...
      <div class="screen">
        <div class="sel-l">
          ...
          <div class="sel-item">
            ...
            <div class="sel">
              <el-select
                ...
                @change="handleChangePhaseStudyId"
              >
                <el-option
          ...
          <div class="sel-item">
            <span>周次:</span>
            <div class="sel">
              <el-select v-model="dataAll.formData.week_time" class="m-2" placeholder="Select" @change="getStore">
                <el-option v-for="item in weeks" :key="item.value" :label="item.title" :value="item.value"></el-option>
              </el-select>
            </div>
          </div>
        </div>
    ...
    <div class="msg-box">
      <div class="all-lab">
        <p>功能室使用总次数</p>
        <div class="img1">{{ dataAll.total }}</div>
      </div>
      <div class="all-lab">
        <p>功能室利用率</p>
        <div class="img2">
          {{ String(dataAll.avg_rage * 100).match(/^\d+(?:\.\d{0,2})?/)[0] + '%' }}
        </div>
      </div>
    ...
    <div class="table-box">
      <el-table :data="dataAll.tableData" style="width: 100%">
        ...
        <el-table-column align="left" prop="title" label="实验室名称" min-width="18" />
        <el-table-column
          align="left"
          prop="amount"
          label="使用次数"
...
import { getLabUseList, findArea, findSchool, findschoolyear } from '@service/dataAnalysisApi'
import { useStore } from '@store/index'
import { getWeeks } from '@/constants/common'
...
const getFindschoolyear = async () => {
  ...
  try {
    ...
    if (code === 10) {
      ...
      weeks.value = getWeeks(data[0].weeknum)
    }
  } catch (error) {
    console.log(error)
...
const getStore = async () => {
  await getLabUseList(dataAll.formData).then((res: any) => {
    dataAll.total = res.data.total
    dataAll.avg_rage = res.data.avg_rage
    dataAll.tableData = res.data.list
    let list = res.data.merge_list
    let xList = []
    let yList1 = []
    let yList2 = []
    // console.log(list)
    for (let index in list) {
      xList.push(list[index].school_year_semester_list.title)
      yList1.push(list[index].school_year_semester_list.amount)
      yList2.push(list[index].school_week_list.amount)
      // console.log(list[index].school_year_semester_list.title)
    }
    // console.log(xList)
    let chartDom = echart.init(chart.value)
    let arrar = ['本学期使用次数', '第' + dataAll.formData.week_time + '周使用次数']
    // console.log(arrar)
    let option = {
      legend: {
        orient: 'horizontal',
      ...
      series: [
        ...
        {
          data: yList2,
          name: '第' + dataAll.formData.week_time + '周使用次数',
          type: 'bar',
          barWidth: 20,
          color: '#EE9A59',
          itemStyle: {
...
const exportExcel = () => {
  ...
  axios
    .post(
      process.env.VUE_APP_BASE_URL_BASE + '/schooldvc/LaboratorySummary/labUseSummary',
      { token, ...dataAll.formData, is_explode: 1 },
      { responseType: 'blob' }
    )
    .then((res) => {
      const blob = new Blob([res.data]) // 处理文档流
      const fileName = store.state.login.userInfo.school.name + '-' + '实验室使用分析.xlsx'
...
onMounted(() => {
  ...
  //   xAxis: {
  //     type: 'category',
  //     data: ['物理实验室1', '物理实验室2', '物理实验室3', '物理实验室4']
  //   },
  //   yAxis: {
  //     type: 'value'
  //   },
  ...
  .msg-box {
    ...
    .all-lab {
      ...
      & > .img1 {
        ...
        font-size: 40px;
        color: #fff;
        background-image: url('../../../assets/images/laboratory-manage/img1.png');
      ...
      & > .img2 {
        ...
        font-size: 40px;
        color: #05c65d;
        background-image: url('../../../assets/images/laboratory-manage/img2.png');
        background-size: 100% 100%;
      }
    }
    .pie-box {
