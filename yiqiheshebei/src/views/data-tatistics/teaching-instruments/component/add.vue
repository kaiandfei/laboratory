    ...
    <div class="leftArea">
      ...
      <div class="topArea" v-show="conserveShow === false">
        <div class="nav">
          <div class="name">*出借人:</div>
          <div>
            {{ addLendRegistrationList.lender_id }}
          </div>
        </div>
        <div class="nav">
          <div class="name">*出借时间:</div>
        ...
        <div class="tableArea">
          ...
          >
            ...
            <el-table-column label="批次" prop="bacth">
              ...
              <select style="width: 100%" v-model="scope.row.bacth">
                <option v-for="item in optionBatch" :key="item.value" :label="item.label"></option>
              </select>
            </template> -->
              <template #default="scope">
    ...
    <div class="rightArea">
      <div class="infoArea" v-for="item in instrumentList" :key="item.id">
        <div class="detailList">
          ...
          <div class="part" style="width: 20%; height: 150px; float: right">
          ...
          <div class="stockTitle" v-show="item.stockStatus === false" @click="item.stockStatus = true">
            展开库存列表
          </div>
        ...
        <div class="tableArea" v-show="item.stockStatus === true">
          ...
          >
            ...
            <el-table-column label="仪器库" prop="laboratory_name" width="100"></el-table-column>
            <el-table-column label="仪器柜" prop="store_name"></el-table-column>
            <el-table-column label="层数" prop="number_plies">
              <template #default="scope">
                <span>{{ '第' + scope.row.number_plies + '层' }}</span>
            ...
            <el-table-column label="操作" prop="">
              <template #default="scope">
                <span style="cursor: pointer; color: green" @click="addLend(scope.row)">出借仪器</span>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
const dataMap = reactive({
  addLendRegistrationList: addLendInstrumnetInfoData,
...
let { addLendRegistrationList, instrumentList, week, optionUser, optionsLaboratory, storeListBox } = toRefs(dataMap)
...
const search = () => {
  console.log('搜索功能')
  getInstrumentInfo({ keyword: searchValue.value }).then((res) => {
...
const addLend = (info: addLendInfo) => {
  ...
  console.log(addLendRegistrationList.value.list)
}
