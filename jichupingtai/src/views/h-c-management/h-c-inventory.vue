<template>
  ...
  <main>
      ...
      <div class="screen">
        <div class="sel-l">
          <div class="sel-item">
            <span>规格:</span>
            <div class="sel">
              <el-select v-model="selMsg.standards" class="m-2" placeholder="请选择" @change="getStudentList">
    ...
    <el-table ref="multipleTableRef" :data="tableData" stripe style="width: 100%">
      ...
      <el-table-column align="left" property="name" label="危化品名称" min-width="15" />
      <el-table-column align="left" property="standards" label="规格属性" min-width="15" />
...
import { getHC, updateHC, delHC, importHC } from '@service/hcApi/index'
...
// 模板下载
const downloadModel = () => {
  let url = 'https://baseapi.swrtest.com/chemical/HazardousChemicalType/downloadtpl'
  fileDownload(url)
}
// 文件导入
const fileImport = () => {
...
const getStudentList = () => {
  ...
  getHC(selMsg).then((res) => {
    uploadState.value = true
    tableData.value = res.data.list.map((e: any) => {
      e.state = false
      return e
    })
    options.value = res.data.standards
...
// 删除
const delStu = (id: any) => {
  ElMessageBox.confirm('确认删除当前危化品!', '警告', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'error'
  }).then(() => {
    delHC({ id: id }).then((res) => {
      getStudentList()
      if (res.code === 10) {
        ElMessage({
          type: 'success',
