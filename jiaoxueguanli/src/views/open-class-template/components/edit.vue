  ...
  <div>
    <div class="header">
      ...
      <div class="my-btn-box">
        <button class="my-btn" @click="save(forms)">保存</button>
        <button class="my-btn" @click="exportTemplate()">下载模版</button>
        ...
        <el-upload
          ...
          accept=".xls,.xlsx"
        >
          <button class="my-btn">导入</button>
        </el-upload>

        <button class="my-btn cancel" @click="back(forms)">返回</button>
      </div>
    </div>
    <div class="box">
      <div class="line-box">
        ...
        <div class="line" v-for="(item, index) in forms.evaluation_project" :key="index">
          ...
          <div class="rb">
            ...
            <i
              class="el-icon-close"
              v-if="forms.evaluation_project.length > 1"
...
<script>
import { saveTemplate, uploadEvaluationProjectTemplate } from '@/network/open-class'
      ...
      axios
        .post(
          process.env.VUE_APP_APP_BASEURL +
            '/teach/EvaluationProjectTemplate/downloadEvaluationProjectTpl',
          { token },
          { responseType: 'blob' }
        )
        .then((res) => {
