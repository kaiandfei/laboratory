  ...
  <div class="page-sortable">
    <div class="sortable">
      ...
      <div class="draggable-module">
        <draggable v-bind="optionsLeft" :list="tagTableLeft" :move="onMove">
          <transition-group tag="ul">
            <li v-for="(item, index) in tagTableLeft" :key="item.key">
              <sortTable
                :name="item.title"
                :index="index"
                :propsData="item.teachercontent"
                :type="item.template"
          ...
          >
            ...
            <div class="modules">
              ...
              <template v-if="item.template === 'questionVue'">
                ...
                <el-input type="textarea" :rows="2" placeholder="多行输入" disabled>
                </el-input>
              </template>
              ...
              <template v-if="item.template === 'fillVue1'">
                这是一个 <span>填空题</span> 模版
              </template>
              <template v-if="item.template === 'fillVue2'">
                这是一个 <span>填空题</span> <span>填空题</span> <span>填空题</span>
              </template>
              <template v-if="item.template === 'chooseVue'">
                <div>
                  <p>这是一个选择题</p>
                  <p v-for="e in ['A', 'B', 'C', 'D']" :key="e" class="option">
                    这是一个选项{{ e }}
                  </p>
                </div>
              </template>
...
export default {
  ...
  created() {
    ...
    if (this.propsData.reportTempletData) {
      this.tagTableLeft = this.propsData.reportTempletData.map((e, i) => {
        return {
          ...e,
          key: Date.now() + i,
        }
      ...
      this.tagTableLeft = this.tagTableRight.map((e, i) => {
        return {
          ...e,
          key: Date.now() + i,
        }
  ...
  methods: {
    ...
    submitForm() {
      ...
      console.log(
        '🚀 ~ file: edit.vue:197 ~ submitForm ~ tagTableLeft:',
        this.tagTableLeft
      )
