<template>
  <div>
    <div id="all" class="all" v-show="!showSortTable">
              ...
              <div>
                ...
                <ul class="experiment_content">
                  <li
                    ...
                    :class="{ notRequired: stageList.length === 1 }"
                  >
                    学&emsp;&emsp;段
                  </li>
                  <li class="experiment_input">
                    <el-form-item label="" prop="stage">
                      ...
                      <span v-else>{{ stageList[0] ? stageList[0].name : '' }}</span>
                    </el-form-item>
                  </li>
                </ul>
                <ul class="experiment_content">
                  ...
                  <li class="experiment_input">
                    <el-form-item label="" prop="c_subject_id">
                      <el-select v-model="item.c_subject_id" placeholder="请选择">
                        <el-option
                          v-for="(item, index) in $store.state.subjectArr"
                          :key="index"
                          :label="item.name"
                          :value="item.id"
                        />
        ...
        <div class="list-title">
          ...
          >
            <div class="btn-upload">
              ...
              <i
                v-show="percentageValue === 0"
                class="el-icon-circle-plus-outline iconv"
              ></i>
              <div class="text">上传<span v-show="percentageValue !== 0">中</span></div>
              <el-progress
                v-show="percentageValue !== 0"
        ...
        <div class="modules-main">
          <div class="modules-top">
            <div class="row">
              <div class="icon">
                <img
                  :src="require('@/assets/images/common/video-icon-white.png')"
                  alt=""
                />
                <span>视频</span>
              </div>
            ...
            <div class="row">
              <div class="icon">
                <img
                  :src="require('@/assets/images/common/link-icon-white.png')"
                  alt=""
                />
                <span>附件</span>
              </div>
      ...
      <div class="content" v-if="isShowDetail">
        <Detail :item="clickItem" @back="back" />
      </div>
      <div v-if="false" class="recommend">
        <div>
          <div>
            <div class="header">公用备课方案：</div>
            <p
...
import { getExpPlan, getMyPlan } from '../../network/subscribe'
...
export default {
  ...
  created() {
    ...
    // 主页列表跳转修改时重置页面上面距离
    document.getElementById('print-dom').scrollTop = 0
    this.listenerFunction()
    // this.getExpPlan()
    this.gradelist = this.$store.state.gradeArr.map((e, i) => {
      return {
        name: e,
        value: i,
      }
  ...
  methods: {
    ...
    changeFile4(data) {
      console.log('🚀 ~ file: AddExperment.vue:602 ~ changeFile4 ~ data:', data)
      this.file4 = data
      this.back()
    },
    showDetail(item) {
    ...
    setStageList() {
      ...
      let time_ids = this.$store.getters.getUserMsg.phase_study_id.split(',')
      console.log('🚀 ~ file: AddExperment.vue:926 ~ setStageList ~ time_ids:', time_ids)
      this.stageList = [
        {
          name: '小学',
          value: 1,
        ...
        return time_ids.includes(e.value.toString())
      })
      // }
      // if (
      //   this.gradelist.find((e) => {
      //     return e <= 6
      //   })
      // ) {
      //   this.stageList.push({
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.$store.state.userMsg.role_id === 3) {
            if (this.item.is_use === 0) {
              this.save(false)
            } else {
              this.$confirm(
                '已在计划中，直接编辑保存会影响教师备课！请与教师提前沟通，是否继续',
                '提示',
                {
    ...
    getDetail(id) {
      ...
      findDetail(data).then((res) => {
        ...
        if (id !== 0) {
          ...
          console.log(
            '🚀 ~ file: AddExperment.vue ~ line 300 ~ findDetail ~ res.data',
            res.data
          )
        }
        ...
        if (!this.item.number) {
          ...
          this.fileList.push(this.item.cover_id)
        }

        const grade = this.item.grade
        // if (grade) {
        //   const gradeList = this.$store.getters.getUserMsg.grade
        //   if (grade <= 6) {
        //     this.item.stage = 1
        //     this.gradelist = gradeList.filter((e) => {
        //       return e <= 6
        //     })
        ...
        //   }
        // }
        this.$set(this.item, 'stage', this.item.stage ? this.item.stage : 1)
        if (grade) {
          if (grade <= 6) {
            this.item.stage = 1
          } else if (grade > 6 && grade <= 9) {
    ...
    show() {
      console.log(this.$refs.h5p_1.fileList)
      console.log(this.$refs.h5p_2.fileList)
      console.log(this.$refs.h5p_3.fileList)
    },
    cancel() {
      if (this.copydata.copy || this.copydata.copyId !== -1) {
        this.$router.go(-1)
      } else {
    ...
    beforeAvatarUploadAll(file, index) {
      ...
      console.log(
        '🚀 ~ file: AddExperment.vue:1070 ~ beforeAvatarUploadAll ~ index:',
        index
      )
      // console.log('🚀 ~ file: h5p.vue ~ line 300 ~ beforeAvatarUpload ~ file', file)
      if (
        !/\.(mp4|avi|gif|jpg|jpeg|png|doc|docx|ppt|pptx|pdf|xls|xlsx)$/i.test(file.name)
      ) {
        this.$message.error(
    ...
    handleUploadProgress(event) {
      // console.log(
      //   '🚀 ~ file: AddExperment.vue:1134 ~ handleUploadProgress ~ event:',
      //   event
      // )
      // 计算上传进度
      ...
      this.percentageValue =
        progress > this.percentageValueMax ? this.percentageValueMax : progress
    ...
    save(type) {
      // type: false: 保存；true:另存为
      this.item.copy = false
      if (this.copydata.copy || type) {
        console.log(this.copydata.copy)
        if (this.copydata.copyId !== -1) {
          this.item.is_exp_plan_detail_id = this.copydata.copyId
        }
        this.item.copy = true
        this.item.member_id = this.$store.state.userMsg.id
      }
      ...
      this.item.is_recommend = 2
      this.item.sections = []
        .concat(this.fileList1)
        .concat(this.fileList2)
        .concat(this.fileList3)
      // .push(this.file4)
      ...
      if (type === true) {
        ...
        Experiment(this.item)
          .then((res) => {
            if (res.code === 10) {
              if (this.copydata.copyId !== -1) {
                this.$router.push({ name: 'ExperimentPlan', params: { id: res.data.id } })
              } else {
                this.$emit('back', 'save')
                this.$emit('jumpTo', res.data.id)
              }
    ...
    select(val) {
      console.log('🚀 ~ file: AddExperment.vue:1046 ~ select ~ val:', val)
    },
    instrument(data) {
