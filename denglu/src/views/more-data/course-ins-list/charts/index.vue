<template>
  <div class="flex">
    <div class="flex-content">
      <div class="tab-box">
        <div
          v-for="(item, index) in tabList.filter((e) => e.value !== 5)"
...
import { CreateInstrumentDiagram, createDiagramComponent } from '@/service/more-data'
...
const dataFuc = async (type: number, i: number) => {
  ...
  let resData = await CreateInstrumentDiagram(searchFormSave.value[i])
  ...
  if (obj) {
    ...
    resData.data.name.forEach((e: any, ii: number) => {
      let need100Flag = [1, 2, 3, 4].includes(obj.value)
      tableData.push({
        name: e,
...
const getCharts = async () => {
  ...
  while (i < checkBoxList.value.length) {
    if (checkBoxList.value[i].check) {
      searchForm.value.list.push(checkBoxList.value[i].value)
      searchForm.value.is_must.push(checkBoxList.value[i].is_must)
    }
    i++
  }
...
const addComponent = (item: any, index: number) => {
  ...
  createDiagramComponent({
    ...
    api_url: '/dateCenter/InstrumentDataCenter/CreateInstrumentDiagram',
    param: {
      ...searchFormSave.value[index],
      y_row: item.type,
      menuIndex: 2,
      need100Flag: [1, 2, 3, 4].includes(item.type) ? 1 : 2
    },
